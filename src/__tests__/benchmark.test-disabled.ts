...
const BENCHMARK_OPTIONS = [
  { format: TextFormat.bitmarkPlusPlus, location: TextLocation.body },
  { format: TextFormat.bitmarkPlusPlus, location: TextLocation.tag },
  { format: TextFormat.text, location: TextLocation.body },
  { format: TextFormat.text, location: TextLocation.tag },
];
...
describe('Breakscape Performance Benchmark', () => {
        ...
        Object.entries(TEST_DATA).forEach(([testName, testData]) => {
          it(`should produce identical results for ${testName}`, () => {
            ...
            if (Array.isArray(charResult)) {
              ...
              regexUnbreakscape = regexImpl.unbreakscape(
        ...
        describe('breakscape() performance', () => {
          Object.entries(TEST_DATA).forEach(([testName, testData]) => {
            it(`should benchmark ${testName} breakscape`, () => {
              const iterations =
                ITERATIONS[testName as keyof typeof ITERATIONS];
  ...
  describe('Edge Cases Performance', () => {
    const edgeCases = {
      ...
      mixedContent:
        'Normal text with [.some] **special** chars ^ and more text',
    };
